...
//    self.bundleIdTextfield.text = self.bundleId;
...
//    NSString *fileString = [ChageLogoMobileconfig addConfigIntoGroupWithConfigs:self.bundleId appSetName:[NSString stringWithFormat:@"%@的桌面logo描述文件",appName] uuid:uuid2];

    NSLog(@"here is the file String: %@", self.bundleId);
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:path]) {
        [[NSFileManager defaultManager] createFileAtPath:path contents:[self.bundleId dataUsingEncoding:NSUTF8StringEncoding] attributes:nil];
    }else{
        [self.bundleId writeToFile:path atomically:YES encoding:NSUTF8StringEncoding error:nil];
    }
